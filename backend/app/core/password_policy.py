...
转译。最小长度 ≥ 8，≥ 3 类字符（小写 / 大写 / 数字 / 符号），且不能命中弱密码
表（不区分大小写）。

字符类要求：NIST SP 800-63B 强调「熵」而非「复杂度」——8 字符是 NIST 推荐底线，
配合「3 类以上」是 mixin 友好的折中：纯字母数字口令（8 字符 random）约 48 bits，
比 8 字符同种字符（27 bits）显著更高，但又不强求符号（符号在很多密码管理器
...
# 长度下限 8：NIST SP 800-63B / NIST2024 推荐 ≥ 8 字符的密码外加无复杂度要求；
# 本系统作为会话凭证 + 改密可吊销，独立要求 ≥ 8 是 user-friendly 折中——比 6 安全、
# 比 12 易用，能让用户手工输入的口令无感通过。
MIN_LENGTH = 8
