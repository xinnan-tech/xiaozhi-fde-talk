...
OCR 在「系统配置 → OCR」分组里配。**只在访谈涉及「从图片里抽文字」时才需要**——纯语音访谈可以**完全跳过**这一节，相关功能不会受影响。

本项目支持的 OCR 类型：

- **baidu**：百度智能云 OCR（推荐国内；按次计费，每月都有免费额度）
- **openai**：OpenAI 兼容 OCR（如 gpt-4o vision）
...
2. 百度账号登录，完成实名认证（个人身份证或企业营业执照）
3. 进入 [公有云服务 → 应用管理](https://console.bce.baidu.com/ai-engine/ocr/app/list)，点「创建应用」
...
参见 [LLM 配置](llm-config.md) 第 3 节「OpenAI（海外）」的注册流程。API Key 共用 OpenAI 平台（也可以用 OpenRouter、Azure OpenAI 等兼容端点）。
