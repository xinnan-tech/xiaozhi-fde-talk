...
def test_validate_value_rejects_jwt_expire_zero_with_i18n():
    ...
    assert ei.value.code == Keys.CONFIG_INVALID_NUMERIC.value
...
def test_i18n_numeric_message_translates_in_each_locale():
    """4 个 locale 都已注册 config.invalid_numeric。"""
    from app.core.i18n.translator import t

    for locale in ("zh-CN", "zh-TW", "en-US", "vi-VN"):
        msg = t(
            Keys.CONFIG_INVALID_NUMERIC.value,
            locale=locale,
        ...
        # 必须非空且包含关键内容
        assert msg
        assert "auth.jwt_expire_minutes" in msg