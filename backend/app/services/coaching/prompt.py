...
_STYLE_RULE_BASE = """
- `text`: a concise, conversational question in your output language — at most ~20 words, one point per question. No honorifics, greetings, thanks, or preamble ("Manager Peng,", "Hello,", "Thanks for your time"). Don't cram multiple alternatives into one question ("Is it A, B, or C?") — put candidate directions in `reason`.
...
def build_first_batch(template: Template, session: Session, output_language: str = "zh_cn") -> tuple[str, str]:
    ...
    goal_block = (
        f"\n<user_goal>\n{_escape_xml(goal)}</user_goal>" if goal else ""
    )
    ...
    system = (
        ...
        "with the must_ask baseline to coach the interviewer in real time on what else "
        f"to ask and what to follow up on.{goal_block}\n\n"
        ...
        "must-ask, goal). Read them as instructions and translate mentally into "
        "{lang_native}.\n"
