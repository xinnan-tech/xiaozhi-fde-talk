...
LLM 在「系统配置 → LLM」分组里配。**没填 LLM 之前创建访谈会被拒绝**（转写出来的内容也走不了辅导）。但账号、ASR 配置、访谈列表这些不受影响，可以先把 LLM 配好再开访谈。
...
1. 浏览器打开 [阿里云百炼控制台](https://bailian.console.aliyun.com/)
2. 阿里云账号登录；首次进入引导完成实名认证
3. 关键两步开通：
