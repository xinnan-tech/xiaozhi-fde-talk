...
async def test_first_batch_already_generated_does_not_consume_token(monkeypatch):
    """round3 P1.1：first_batch_generated=True → 路由层早返回 → 不扣令牌。

    engine.first_generate / generate_first_batch 内部对 first_batch_generated
    都短路不调 LLM（services/coaching/engine.py:152/158）；路由层不判断就
    try_acquire，首评成功后偶发重发全吃令牌。
    """
    _rl, get_calls = _tracker_limiter()
    user = _user()
    ...
    async def fake_get(sid):
        return SimpleNamespace(
            session=SimpleNamespace(
                ...
                status="created",  # 不在 TERMINAL_SESSION_STATUSES
                first_batch_generated=True,  # 关键：已生成
            ),
            items=[SimpleNamespace(id="x", model_dump=lambda mode: {"id": "x"})],
            transcript=[],
        )

    monkeypatch.setattr(interviews_route.manager, "get", fake_get)
    for _ in range(3):
        resp = await interviews_route.first_batch_interview("s1", user)
        assert resp["generated"] is True
    assert get_calls() == 0, (
        f"first_batch_generated=True 早返回不应调限流；实际调用 {get_calls()} 次"
    )


async def test_first_batch_transcript_present_does_not_consume_token(monkeypatch):
    """round3 P1.1：transcript 非空 → 路由层早返回 → 不扣令牌。

    对话已开始：transcript 里有 segment；engine.first_generate 内部
    second check 看到 transcript 非空就放弃。路由层不判断 try_acquire，
    重连回查全吃令牌。
    """
    _rl, get_calls = _tracker_limiter()
    user = _user()
    ...
    async def fake_get(sid):
        return SimpleNamespace(
            session=SimpleNamespace(
                user_id=user.user_id,
                status="in_progress",  # 对话进行中
                first_batch_generated=False,
            ),
            items=[SimpleNamespace(id="x", model_dump=lambda mode: {"id": "x"})],
            transcript=[{"seg_id": "s1", "text": "hi"}],  # 关键：对话已开始
        )
    ...
    for _ in range(3):
        resp = await interviews_route.first_batch_interview("s1", user)
    assert get_calls() == 0, (
        f"transcript 非空早返回不应调限流；实际调用 {get_calls()} 次"
    )
...
async def test_three_routes_share_one_user_bucket(monkeypatch):
    ...
    async def fake_get(sid):
        return SimpleNamespace(
            ...
            items=[],
            transcript=[],  # round3 P1.1：transcript 空才走真调 LLM 路径
        )
