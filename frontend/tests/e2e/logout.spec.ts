...
// 已登录 → 点 .user-avatar → ElMessageBox 确认 → userStore.logOut()
// → removeToken() 清 localStorage[user-info] 与 cookie[authorized-token]
// → router.push("/home") → 再点 .user-avatar 触发 login-dialog
// （home 视图不自动弹登录框，见 redirect.spec.ts）
...
test("已登录用户点登出后 token 清掉、再点 avatar 弹登录框", async ({ page }) => {
  ...
  // 点 avatar 直接弹 ElMessageBox 确认（home/index.vue:202 不走中间 dropdown）
  await page.locator(".user-avatar").click()
  const confirm = page.locator(".el-message-box__btns button.el-button--primary")
